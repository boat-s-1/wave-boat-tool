...
def show_rank_card(rank, boat, percent, detail=None, is_double_circle=False):
    ...
    if percent >= 30:
        ...
        shadow = "0 0 10px rgba(255,105,180,0.6)"
        badge = "🌸 おすすめ"
...
with tab2:
    ...
    detail_scores = {}
    for b in boats:
        detail_scores[b] = (detail[b]["motor"]*wm + detail[b]["local"]*wl + (1/detail[b]["start"])*ws + (1/detail[b]["expo"])*we)
...
with tab3:
    ...
    canvas=st_canvas(drawing_mode="transform", background_image=bg, initial_drawing=init_draw, height=500, width=360, update_streamlit=True, key="canvas")
